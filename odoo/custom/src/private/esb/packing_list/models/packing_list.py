...
class ESBPackingList(models.Model):
    ...
    def _get_default_partner_id(self):
        if self._context.get("default_picking_ids"):
            pickings = self.env["stock.picking"].browse(
                self._context["default_picking_ids"]
            )
            partners = pickings.mapped("partner_id")
            if len(partners) != 1:
                raise ValidationError(
                    _("Selected pickings do not belong to the same partner!")
                )
    ...
    def _get_default_ship_form(self):
        if self.company_id:
            return ", ".join(
                [self.company_id.state_id.name, self.company_id.country_id.name]
            )
        return False
    ...
    def _get_default_ship_to(self):
        if self.partner_id:
            return ", ".join(
                [self.partner_id.state_id.name, self.partner_id.country_id.name]
            )
        return False
                    ...
                    lines.append(
                        (
                            ...
                            0,
                            {
                                ...
                                "package": move.product_uom_qty
                                / move.product_id.pack_carton,
                                "net_weight": move.product_uom_qty
                                * move.product_id.weight,
                                "gross_weight": move.product_uom_qty
                                * move.product_id.gross_weight,
                            },
                        )
                    )
